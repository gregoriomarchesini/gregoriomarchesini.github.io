...
import teachingMd from "./teaching.md?raw";

function parseTeachingMarkdown(md: string): TeachingSection[] {
  const lines = md.replace(/\r\n/g, "\n").split("\n");
  const sections: TeachingSection[] = [];
  let current: TeachingSection | null = null;
  let currentRef: Partial<Reference> | null = null;
  ...
  for (let rawLine of lines) {
    ...
    if (headingMatch) {
      if (currentRef && current) {
        const ref = currentRef as Reference;
        if (ref.title && ref.authors) current.refs.push(ref);
        currentRef = null;
      }
      if (current) sections.push(current);
      current = { heading: headingMatch[1].trim(), refs: [] };
      continue;
    }

    // Check for list item start (- title: ...)
    const listMatch = trimmed.match(/^-\s+title:\s*(.+)$/);
    if (listMatch && current) {
      if (currentRef && (currentRef as Reference)?.title) {
        const ref = currentRef as Reference;
        if (ref.title && ref.authors) current.refs.push(ref);
      }
      currentRef = { title: listMatch[1].trim(), authors: "", type: "book" };
      continue;
    }

    // Check for indented keys (authors:, link:, description:, etc.)
    const keyMatch = trimmed.match(/^(authors|link|description):\s*(.*)$/);
    if (keyMatch && currentRef) {
      const key = keyMatch[1];
      const value = keyMatch[2].trim();
      if (key === "authors") {
        currentRef.authors = value;
      } else if (key === "link") {
        currentRef.url = value || undefined;
        // Determine type based on URL
        if (value && /youtu(?:be\.com|\.be)\//.test(value)) {
          currentRef.type = "video";
        } else if (value) {
          currentRef.type = "article";
        }
      } else if (key === "description") {
        currentRef.description = value || undefined;
      }
    }
  }

  // Push final ref and section
  if (currentRef && current) {
    const ref = currentRef as Reference;
    if (ref.title && ref.authors) current.refs.push(ref);
  }
...
export const teachingSections: TeachingSection[] = parseTeachingMarkdown(teachingMd as string);
